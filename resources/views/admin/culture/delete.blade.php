...
@section('selectClass','cultureDelete')
@section('admin')
  <div style="overflow: scroll; height: 900px" id="newSelect">
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="#">文化遗产</a></li>
        <li class="breadcrumb-item active" aria-current="page">文化遗产回收站</li>
      </ol>
    </nav>
          ...
          @foreach($data as $item)
            <li class="item d-flex flex-row shadow-sm p-2 mb-3 bg-white rounded border border-info rounded">
              ...
              <div class="item-content">
                ...
                <div>
                  <a href="/culture/info/{{$item->id}}" class="btn btn-primary">预览</a>
                  <a href="/culture/delete/{{$item->id}}/1" class="btn btn-success" onclick="return confirm('是否恢复')">恢复文章</a>
                  <a href="/culture/delete/{{$item->id}}/2" class="btn btn-danger" onclick="return confirm('是否永久删除这条数据')">永久删除</a>
                </div>
              </div>
            </li>
          @endforeach
